# -*- coding: utf-8 -*-
"""
Saqer — Maker-Only (Bitvavo / EUR) — Telegram /webhook (Arabic-only)
الأوامر (نص عربي فقط):
- "اشتري COIN [EUR]"
- "بيع COIN [AMOUNT]"
- "الغ COIN"
...
# كاش للماركتات + تتبع أوامر الشراء المفتوحة
...
def load_markets_once():
    """يجلب /markets مرّة واحدة ويخزّن عدد المنازل العشرية بدقة."""
    ...
    for r in rows:
        ...
        if not base or not market:
            continue
        price_dec = int(r.get("pricePrecision", 6) or 6)     # عدد منازل عشرية للسعر
        amt_dec   = int(r.get("amountPrecision", 8) or 8)    # عدد منازل عشرية للكمية
        step      = Decimal(1) / (Decimal(10) ** amt_dec)
        meta[market] = {
...
def _price_decimals(market: str) -> int:
    return int(_meta(market).get("priceDecimals", 6))

def _amount_decimals(market: str) -> int:
    return int(_meta(market).get("amountDecimals", 8))
...
def fmt_price_dec(market: str, price: float | Decimal) -> str:
    """تنسيق السعر بعدد منازل عشرية ثابت (ROUND_DOWN)."""
...
def fmt_amount(market: str, amount: float | Decimal) -> str:
    """تنسيق الكمية وفق amountDecimals + step (ROUND_DOWN)."""
...
def _poll_status_until(market: str, orderId: str, deadline_ts: float,
    ...
    """نرجع (ok, status, last_state) — ok فقط لو canceled/filled."""
    last = None
...
def tg_webhook():
    ...
    try:
        ...
        if low.startswith("اشتري"):
            parts = text.split()
            if len(parts) < 2:
                tg_send("صيغة: اشتري COIN [EUR]")
                return jsonify(ok=True)
            market = _norm_market_from_text(parts[1])
            if not market:
                tg_send("⛔ عملة غير صالحة.")
                return jsonify(ok=True)
            eur = None
            if len(parts) >= 3:
        ...
        if low.startswith("الغ"):
            ...
            if not info:
                tg_send("لا يوجد أمر مفتوح لهذه العملة.")
                return jsonify(ok=True)
...
def home():
    return "Saqer Maker — Arabic-only (/webhook) ✅"
