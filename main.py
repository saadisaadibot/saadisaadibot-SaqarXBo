...
- إصلاحات نهائية: mapping صحيح لـ precision، عدم استخدام amountQuote للـ limit،
  حماية رصيد + backoff عند 216، تهدئة المحاولات وعدم إعلان فشل إلا بعد محاولات حقيقية.
- أوامر تيليجرام: /summary /close /enable /disable
- إشعار الشراء فقط عبر /hook من بوت الإشارة.
"""
...
# لوج المحاولات التفصيلي (افتراضي مغلق)
VERBOSE_TRIES = os.getenv("VERBOSE_TRIES","0") == "1"
...
IB_BACKOFF_TRIES    = int(os.getenv("IB_BACKOFF_TRIES", "6"))       # محاولات
...
MAKER_WAIT_BASE_SEC   = 60       # صبر ابتدائي أعلى شوي
MAKER_WAIT_MAX_SEC    = 420
MAKER_WAIT_STEP_UP    = 20
MAKER_WAIT_STEP_DOWN  = 10
BUY_MIN_EUR           = 5.0
WS_STALENESS_SEC      = 2.0

# مهلة حقيقية بين محاولات وضع الأمر (مع jitter)
TRY_SLEEP_BASE        = 0.9
TRY_SLEEP_SPREAD      = 0.8
...
MARKET_META    = {}   # "ADA-EUR" -> {"minQuote","minBase","tick","step","pdec","adec"}
_ws_prices     = {}
_ws_lock       = Lock()
...
def _sleep_try():
    time.sleep(max(0.1, TRY_SLEEP_BASE + (random.random()-0.5)*TRY_SLEEP_SPREAD))
...
# ========= Markets / Meta =========
def _to_step_from_precision(x, default_decimals):
    """
    Bitvavo يرجّع pricePrecision/amountPrecision كعدد خانات (integer).
    حوّلها إلى خطوة: 10^-precision
    وإذا رجع خطوة جاهزة (نادرًا)، صحّحها.
    """
    try:
        if x is None: return 10 ** (-default_decimals)
        xv = float(x)
        # إذا xv >= 1 فهذا غالبًا "عدد خانات" (مثلاً 5) → step=10^-5
        if xv >= 1:
            return 10 ** (-int(round(xv)))
        # إذا <1 فهي أصلاً خطوة
        return xv
    except:
        return 10 ** (-default_decimals)
...
def load_markets():
    ...
    try:
        ...
        for r0 in rows:
            ...
            if not base or quote!="EUR": continue

            # precision → step/decimals (مصوب)
            pdec = int(r0.get("pricePrecision", 6) or 6)
            adec = int(r0.get("amountPrecision", 8) or 8)
            tick = _to_step_from_precision(r0.get("pricePrecision", 6), 6)
            step = _to_step_from_precision(r0.get("amountPrecision", 8), 8)
...
def _round_price(market, price):
    ...
    tick = float(meta.get("tick", 1e-6))
    p    = _round_to_step(price, tick)
    # لا ننقص عن tick
    return max(tick, p)

def _round_amount(market, amount):
    meta = MARKET_META.get(market, {}) or {}
    step = float(meta.get("step", 1e-8))
    a    = _round_to_step(amount, step)
    return max(step, a)

def _format_price(market, price) -> str:
    meta = MARKET_META.get(market, {}) or {}
    pdec = int(meta.get("pdec", 6))
    return f"{_round_price(market, price):.{pdec}f}"

def _format_amount(market, amount) -> str:
    meta = MARKET_META.get(market, {}) or {}
    adec = int(meta.get("adec", 8))
    return f"{_round_amount(market, amount):.{adec}f}"
...
def relax_patience_on_success(market):
    try:
        cur = get_patience_sec(market)
        base = MAKR_WAIT_BASE_SEC if False else MAKER_WAIT_BASE_SEC
...
def open_maker_buy(market: str, eur_amount: float):
    """Maker postOnly على أفضل Bid، مع تجميع partial fills + حماية رصيد + backoff وتهدئة المحاولات."""
    # --- 1) تحديد مبلغ الشراء الحقيقي ---
    ...
    placed_orders = 0   # أوامر تم إرسالها فعليًا (POST)
    inner_attempts= 0   # عدد محاولات POST (حتى لو رجع خطأ فورًا)
    ...
    try:
        while (time.time() - started) < patience and remaining_eur >= (minq * 0.999):
            ...
            # سعر Maker للشراء: تحت ask قليلاً وعلى bid
            ...
            if last_order:
                ...
                # أعِد التسعير إذا bid تغيّر بفارق معتبر
            ...
            # ضع أمر جديد (amount محسوبة من EUR) مع backoff وتهدئة
            if not last_order and remaining_eur >= (minq * 0.999):
                ...
                while attempt < IB_BACKOFF_TRIES and remaining_eur >= (minq * 0.999):
                    ...
                    if VERBOSE_TRIES:
                        ...
                        send_message(f"🧪 محاولة شراء #{inner_attempts}: amount={_format_amount(market, amt_base)} | سعر≈{_format_price(market, cur_price)} | EUR≈{exp_quote:.2f}")
                    ...
                    # خطأ رصيد → قلّص المبلغ وحاول بعد مهلة
    ...
    if not all_fills:
        ...
        # لا نعلن فشل إلا إذا حصلت محاولات فعلية أو انتهت مهلة معتبرة
        if inner_attempts > 0 or elapsed >= (MAKER_REPRICE_EVERY*2):
            send_message(f"⚠️ لم يكتمل شراء Maker.\n• أوامر موضوعة: {placed_orders}\n• محاولات داخلية: {inner_attempts}\n• زمن: {elapsed:.1f}s\nسنعزّز الصبر تلقائيًا وسنحاول لاحقًا.")
        return None
...
def telegram_webhook():
    try:
        ...
        if text.startswith("/start"):
            send_message("مرحباً! أوامر: /summary /close /enable /disable")
        ...
        print("TG webhook err:", e); return jsonify(ok=False), 200

# ========= Webhook من بوت الإشارة =========
...
def hook():
    ...
    try:
        ...
        if cmd in ("summary","/summary"):
            txt = build_summary()
            send_message(txt)
            return jsonify({"ok":True,"summary":txt})
        ...
        if cmd in ("close","sell","exit"):
            ...
            if has_position:
                do_close_maker("Manual")
                return jsonify({"ok":True,"msg":"closing"})
            else:
                send_message("لا توجد صفقة لإغلاقها.")
                return jsonify({"ok":False,"err":"no_active_trade"})
